...
bool Bishop::isOutside(int X, int Y) {
	return X < 0 || X >= 8 || Y < 0 || Y >= 8;
}
bool Bishop::isSameColor(bool color, int X, int Y, BoardControl& board) {
	return board.GetPieceID(X, Y) != Empty and board.getPieceColor(X, Y) == color;
}
std::vector<pair> Bishop::getPossibleMove(bool color, int row, int col, BoardControl& board) {
	std::vector<pair> tmp;
...
bool Bishop::isValidMove(int X, int Y, int toX, int toY, BoardControl& board) {
...
bool Bishop::canCheck(int X, int Y, int toX, int toY, BoardControl& board) {
